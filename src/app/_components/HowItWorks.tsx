const STEPS = [
  {
    n: "01",
    h: "Алиса подбирает",
    p: "Пара вопросов о возрасте, самочувствии и истории — и вы получаете набор анализов под себя, а не общий список.",
  },
  {
    n: "02",
    h: "Сдаёте, как удобно",
    p: "В партнёрской лаборатории рядом с домом или дома — медсестра приедет и возьмёт кровь.",
  },
  {
    n: "03",
    h: "Результаты в приложении",
    p: "Расшифровка простым языком и референсы. В следующий раз всё ложится в одну историю — видно динамику.",
  },
  {
    n: "04",
    h: "Разбор с врачом",
    p: "По видео или очно в партнёрской клинике. Врач подберёт добавки — привезём по подписке — и направит к специалисту, только если это нужно.",
  },
];

export function HowItWorks() {
  return (
    <section id="how" className="paper">
      <div className="wrap">
        <div className="sec-head">
          <div className="sec-eyebrow">Как это работает</div>
          <h2>От «не знаю, что сдать» — до понятного результата и врача</h2>
          <p>Четыре шага. Мы закрываем весь путь: вам не нужно самим гадать, какие анализы выбрать.</p>
        </div>
        <div className="steps">
          {STEPS.map((s) => (
            <div className="step" key={s.n}>
              <div className="num">
                {s.n}
                <span className="ln" />
              </div>
              <h3>{s.h}</h3>
              <p>{s.p}</p>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
